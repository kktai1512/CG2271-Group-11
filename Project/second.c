...
#define ECHO_IN 2 // PortA Pin 2
#define TRIG_OUT 5 // PortD Pin 5
...
void initTimer(void) {
	...
	// Start timer
	TPM2_C1SC |= (TPM_CnSC_ELSA(1) | TPM_CnSC_ELSB(1)); //Falling and Rising Edge Input Capture
	TPM2->CNT = 0;
	
	
	TPM2_C1SC |= TPM_CnSC_CHIE(1); // Enable Channel interrupt
...
void TPM2_IRQHandler(void) {
	...
	if (TPM2_C1SC & TPM_CnSC_CHF_MASK) {
		...
		if (active) {
			...
			distance = ((TPM2_C1V) * multiplier)/2;
			resetTimer();
		}
		else {
			active = 1;
